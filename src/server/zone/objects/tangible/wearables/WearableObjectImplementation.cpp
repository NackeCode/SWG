...
void WearableObjectImplementation::generateSockets(CraftingValues* craftingValues) {
	...
	int assembly_skill = 0;
	int force_assembly = 0;
	int force_luck = 0;
	int assembly_food = 0;
	...
	if (craftingValues != nullptr) {
		...
		if (manuSchematic != nullptr) {
			...
			if (player != nullptr && draftSchematic != nullptr) {
				...
				if (assemblySkill == "armor_assembly" || assemblySkill == "droid_assembly") { // socket mechanic change for armorsmith and droid engineer (cybernetic armor) only

			const BuffList* buffList = player->getBuffList(); // these next ten lines check for assembly food buff and use value for socket calculation
			for(int i = 0; i < buffList->getBuffListSize(); ++i) {
				...
				for(int j = 0; j < skillMods->size(); ++j) {
					...
					if (name == "craft_bonus") {
						assembly_food = skillMods->elementAt(j).getValue();
						j = skillMods->size();
						i = buffList->getBuffListSize();
					}
				}
			}
				assembly_skill = player->getSkillMod(assemblySkill); // 100 for master profession, 25 more with tapes, 10 with profession related building bonus
				force_luck = player->getSkillMod("force_luck"); // 0-4 force sensitive skill
				force_assembly = player->getSkillMod("force_assembly"); // 0-20 force sensitive skill

				generatedCount = int((1.5*assembly_food + 1.3*std::max(assembly_skill-50,0) + 2.5*force_assembly
				 		+ System::random(System::random(25*force_luck)) 
						+ std::max(int(System::random(std::max(450-int(std::max(assembly_skill-50,0) + 2.5*force_assembly),0))-30),0))/100);

				if (craftingValues->hasProperty("sockets") && craftingValues->getCurrentValue("sockets") > 0)
					generatedCount += craftingValues->getCurrentValue("sockets");
				}
				else { // old socket generation code left for tailors and non-armor related professions
					assembly_skill = player->getSkillMod(assemblySkill) * 2.5; // 0 to 352 (ish) max
					force_luck = System::random(player->getSkillMod("luck")
							+ player->getSkillMod("force_luck"));

					int random = (System::random(750));

					float roll = System::random(assembly_skill + force_luck + random);

					generatedCount = int(float(MAXSOCKETS * roll) / float(MAXSOCKETS * 100));
				}
			}
		}
	}
